 ...
 */
	...
	var timer;
jQuery(function() {
	var $slide10utab = jQuery('#slide10utab');
	var $slide10ubgimg = jQuery('#slide10ubgimg');
	var i = 1;
	$slide10utab.children('li').each(function() {
		jQuery(this).attr('ref', i);
		i++
	});
	i = 1;
	$slide10ubgimg.children('li').each(function() {
		jQuery(this).attr('ref', i);
		i++
	});
	$slide10utab.children('.slide-back').fadeOut(0);
	jQuery('#slide10ubgimg li div').attr('load', 0);
	loadBg();
	jQuery('#slide10ubgimg li div').ready(function() {
		jQuery('#slide10ubgimg li div').attr('load', 1)
	});
	$slide10utab.children('li').hover(function() {
	...
	jQuery('.slide10u .tabs li').click(function() {
		...
		jQuery('.slide10u').append($bg2);
		jQuery('.bgimg li').fadeOut(0);
		jQuery('.bgimg li[ref=' + jQuery(this).attr('ref') + '] div').each(function() {
			jQuery(this).animate({
				opacity: '0'
			},
			0)
		...
		if (jQuery('.bgimg li[ref=' + jQuery(this).attr('ref') + '] img').attr('load') == '0') {
			clearInterval(timer);
			jQuery('#load').fadeIn(200)
		} else {
			jQuery('.bgimg li[ref=' + jQuery(this).attr('ref') + ']').fadeIn(0);
			jQuery('#load').fadeOut(200)
		}
		current = parseInt(jQuery(this).attr('ref')) + 1
	});
	_count = jQuery('.slide10u .tabs li').length;
	_timer();
	timer = setInterval('_timer()', 5000);
	timer2 = setInterval(function() {
		jQuery('.bgimg2').remove()
	},
	bgimg_remove)
});
function _timer() {
	if (current > _count) current = 1;
	jQuery('#load').fadeOut(200);
	jQuery('.tabs li').children('.slide-back').removeClass('active');
	jQuery('.tabs li[ref=' + current + ']').children('.slide-back').addClass('active');
	jQuery('.bgimg li').siblings().fadeOut(300);
	var $bg2 = jQuery('.bgimg').clone();
	$bg2.attr({
		'class': 'bgimg2'
	});
	jQuery('.slide10u').append($bg2);
	jQuery('.bgimg li').fadeOut(0);
	jQuery('.bgimg li[ref=' + current + '] div').each(function() {
		jQuery(this).animate({
			opacity: '0'
		},
		0)
...
function randomFadeOut() {
	var randomTime;
	var active = jQuery('.active').parent('li').attr('ref');
	jQuery('.bgimg li[ref=' + active + '] div').each(function() {
...
function loadBg() {
	...
	var left;
	var top;
	jQuery('.bgimg li div').each(function() {
